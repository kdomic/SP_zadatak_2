...
int diff(char a[], char b[]){ //manj,veci
    char t[5],_t[5];
    int d,m,y,d1,d2;
    t[0] = a[0]; t[1] = a[1]; d = atoi(t);
    t[0] = a[2]; t[1] = a[3]; m = atoi(t);
    t[0] = a[4]; t[1] = a[5]; t[2] = a[6]; t[3] = a[7]; y = atoi(t);
    d1 = d + m*30+(y-2010)*360;
    
    _t[0] = b[0]; _t[1] = b[1]; d = atoi(_t);
    _t[0] = b[2]; _t[1] = b[3]; m = atoi(_t);
    _t[0] = b[4]; _t[1] = b[5]; _t[2] = b[6]; _t[3] = b[7]; y = atoi(_t);
    d2 = d + m*30+(y-2010)*360;
...
void istovar2(tlist *list){
     ...
     int d = diff(data.datum,data.rok);
     PopS(list);
     if(!IsEmptyS(list))
        istovar2(list);
     if(d<=14)
